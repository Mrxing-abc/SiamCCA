...
from SiamCCA.rgbt_network import SiamCCANet
from SiamCCA.config import config
from SiamCCA.transforms import ToTensor
from SiamCCA.utils import generate_anchors, get_exemplar_image, get_instance_image, box_transform_inv,add_box_img,add_box_img_left_top,show_image
...
class SiamCCATracker:
    def __init__(self, model_path, cfg=None, is_deterministic=False):
        self.frame_count = 0  # 记录帧数
        self.pscore_1_list = []  # 存储每帧 pscore_1
        self.pscore_2_list = []  # 存储每帧 pscore_2
        self.pscore_3_list = []
        self.name = 'SiamCCA'
        if cfg:
            config.update(cfg)
    ...
    def update(self, rgb_frame, t_frame):
        ...
        # 记录数据
        self.pscore_1_list.append(pscore_1[best_pscore_id_1])
        self.pscore_2_list.append(pscore_2[best_pscore_id_2])
        self.pscore_3_list.append(max(pscore_1[best_pscore_id_1], pscore_2[best_pscore_id_2]))

        plt.figure(figsize=(10, 5))
        plt.plot(range(1, self.frame_count + 1), self.pscore_1_list, label='PredScore1(RGB)', color='b')
        plt.plot(range(1, self.frame_count + 1), self.pscore_2_list, label='PredScore2(T)', color='r')
        # plt.plot(range(1, self.frame_count + 1), self.pscore_3_list, label='Max_pscore', color='r', linestyle='dashed')
        # # 隐藏所有轴线
        ...
        plt.xlabel('Frame')
        plt.ylabel('Score')
        plt.title('Tracking Confidence Score Over Time')
        plt.legend()
        plt.grid()
        plt.xlim(1, self.frame_count)  # 添加这行
        plt.show()



        # print(pscore_1[best_pscore_id_1])
        # print(pscore_2[best_pscore_id_2])
    ...
    def plot_scores(self):
        """ 绘制 pscore_1 和 pscore_2 随帧数的变化曲线 """
        plt.figure(figsize=(10, 5))
        plt.plot(range(1, self.frame_count + 1), self.pscore_1_list, label='pscore_1 (RGB)', color='b')
        plt.plot(range(1, self.frame_count + 1), self.pscore_2_list, label='pscore_2 (Thermal)', color='r')
        plt.xlabel('Frame')
        plt.ylabel('Score')
        plt.title('Tracking Confidence Score Over Time')
        plt.legend()
